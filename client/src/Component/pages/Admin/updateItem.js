...
const UpdateItem = ({el,id}) => {
    ...
    const [update,setupdate]=useState({title:el.title,price:el.price,imageUrl:el.imageUrl,description:el.description})
    // handelInput
    ...
    const updatefood=()=>{
        dispatch (updateItem(id,update.title, update.price,update.imageUrl ,update.description)) 
...
<Modal.Title className="title-modal">Modifier Item</Modal.Title>
</Modal.Header>
<Modal.Body>
<Form >
...
<Form.Control type="text" placeholder="enter image"  name='image'  defaultValue={el.imageUrl}   onChange={handleChange} />
</Form.Group>
